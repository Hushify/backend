...
public static class Move
{
    ...
    {
        var targetFolder = await ctx.Folders.FirstOrDefaultAsync(f => f.Id == req.TargetFolderId, ct);
        if (targetFolder is null)
        {
            throw new AppException("Invalid target folder.");
        }

        var folders = ctx.Folders.Where(f => req.Folders.Select(reqFolder => reqFolder.Id).Contains(f.Id));
        ...
        var files = ctx.Files.Where(f => req.Files.Select(reqFile => reqFile.Id).Contains(f.Id));
        foreach (var file in files)
        {
            var reqFolder = req.Files.First(r => r.Id == file.Id);

            file.ParentFolderId = targetFolder.Id;
            file.MetadataBundle = reqFolder.MetadataBundle;
            file.KeyBundle = reqFolder.KeyBundle;
...
public sealed record MoveRequest(Guid TargetFolderId, NodeToMove[] Folders, NodeToMove[] Files);
